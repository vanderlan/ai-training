...
logger = logging.getLogger("multi_agent.agents")
...
3. Use appropriate formatting (headings, bullet points, etc.)
...
class WorkerAgent:
    ...
    def __init__(self, llm_client: LLMClient, system_prompt: str, name: str):
        self.llm = llm_client
    ...
    def execute(self, task: str, context: str = "") -> str:
        ...
        logger.info("[%s] Executing task: %s", self.name, task[:120])
        ...
        logger.info("[%s] Completed — %d chars response", self.name, len(response))
        return response
